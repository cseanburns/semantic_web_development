...
The `type` attribute will use the JSON-LD [MIME type][mime_mdn], which is `application/ld+json`.
...
Some versions of kibbeh are eaten raw, but baked kibbeh is cooked.
...
is likely created through other automated mechanisms.

## Step 3: Serializing schema.org as JSON-LD

When we create a JSON-LD data model for our web pages, we should use data extracted from the content on the web pages and no more.
...
then we should create a JSON-LD model based on that topic and on the content in that article **only**.
...
search engines may ignore that markup or make the page ineligible for certain rich results.

Here's a basic JSON-LD example that describes the Debian GNU/Linux distribution, one of the topics of my made-up web page.
...
The above example is based on the `SoftwareApplication` type.
...
which itself is a subclass of the `Thing` type, then `SoftwareApplication` can use properties from itself but
...
Since those values can be schema.org `Types`, we can create a nested JSON object:
...
Understanding this requires acquiring a judgment of a thing's [aboutness][aboutness_wiki].
...
The validator will test both the syntax of your JSON-LD and flag invalid schema.org types or properties.
...
Now that I've completed the JSON-LD object that describes one aspect of the topic of my Linux Systems Administration page,
...
Therefore, each web page on a website should include unique JSON-LD.
