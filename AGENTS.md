...
- `src/SUMMARY.md` defines the mdBook chapter order; keep it in sync with new files.
- `epub_gen.sh` in the repo root contains an explicit ordered list for the EPUB; update it when adding or renaming chapters.
- `book/` is generated output from mdBook; do not edit by hand.
- Root assets/config include `book.toml`, `epub.css`, `epub_metadata.xml`, and `semantic_web_development.epub`.
- `src/title.txt` stores the book title used by the build.
